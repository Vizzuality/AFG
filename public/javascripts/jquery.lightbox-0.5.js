	...
	$.fn.lightBox = function(settings) {
					...
					<div id="lightbox-container-image-data">
						...
						<div id="lightbox-secNav">
							<a href="#" id="lightbox-secNav-btnClose">
								<img src="/images/gallery/lightbox-btn-close.gif">
							</a>
						</div>
					</div>
				</div>
		...
		function _set_image_to_view() { // show the loading
			...
			if ( settings.fixedNavigation ) {
				$('#lightbox-image,#lightbox-container-image-data-box,#vertex_bkg,#lightbox-image-details-currentNumber').hide();
			} else {
				// Hide some elements
				$('#lightbox-image,#lightbox-nav,#lightbox-nav-btnPrev,#vertex_bkg,#lightbox-nav-btnNext,#lightbox-container-image-data-box,#lightbox-image-details-currentNumber').hide();
			}
		...
		function _show_image_data() {
			...
			if ( settings.imageArray.length > 1 ) {
				...
				$('#lightbox-image-details-currentNumber').html(settings.txtImage).show();
			}		
		}
		/**
		...
		function _set_navigation() {
			...
			$('#lightbox-nav-btnPrev').css({ 'background' : 'url(' + settings.imageBtnPrev + ') 0 0 no-repeat' });
			$('#lightbox-nav-btnNext').css({ 'background' : 'url(' + settings.imageBtnNext + ') 0 0 no-repeat' });
			...
			if ( settings.activeImage != 0 ) {
				if ( settings.fixedNavigation ) {
					...
					$('#lightbox-nav-btnPrev').unbind().hover(function() {
						$(this).css({ 'background' : 'url(' + settings.imageBtnPrev + ') 0 -35px no-repeat' });
					},function() {
						$(this).css({ 'background' : 'url(' + settings.imageBtnPrev + ') 0 0 no-repeat' });
			...
			if ( settings.activeImage != ( settings.imageArray.length -1 ) ) {
				if ( settings.fixedNavigation ) {
					...
					$('#lightbox-nav-btnNext').unbind().hover(function() {						
						$(this).css({ 'background' : 'url(' + settings.imageBtnNext + ') 0 -35px no-repeat' });
					},function() {
						$(this).css({ 'background' : 'url(' + settings.imageBtnNext + ') 0 0 no-repeat' });
					}).show().bind('click',function() {
						settings.activeImage = settings.activeImage + 1;
						_set_image_to_view();
						return false;
