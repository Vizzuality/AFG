class SpeciesController < ApplicationController
  ...
  DEFAULT_PARAMS = ['action', 'controller', 'taxonomy']
  ...
  def index
    if params[:taxonomy].blank?
      ...
      species = if (params.keys - DEFAULT_PARAMS - AVAILABLE_PARAMS).size > 0
        ...
        @taxonomy = "kingdom"
        @term = params[:kingdom]
        Species.where(:kingdom => params[:kingdom])
      elsif (params.keys & AVAILABLE_PARAMS).size == 2 && params[:kingdom] && params[:phylum]
        @taxonomy = "phylum"
        @term = params[:phylum]
        Species.where(:kingdom => params[:kingdom], :phylum => params[:phylum])
      elsif (params.keys & AVAILABLE_PARAMS).size == 3 && params[:kingdom] && params[:phylum] && params[:t_class]
        @taxonomy = "class"
        @term = params[:t_class]
        Species.where(:kingdom => params[:kingdom], :phylum => params[:phylum], :t_class => params[:t_class])
      elsif (params.keys & AVAILABLE_PARAMS).size == 4 && params[:kingdom] && params[:phylum] && params[:t_class] && params[:order]
        @taxonomy = "order"
        @term = params[:order]
        Species.where(:kingdom => params[:kingdom], :phylum => params[:phylum], :t_class => params[:t_class], :t_order => params[:order])
      elsif (params.keys & AVAILABLE_PARAMS).size == 5 && params[:kingdom] && params[:phylum] && params[:t_class] && params[:order] && params[:family]
        @taxonomy = "family"
        @term = params[:family]
        Species.where(:kingdom => params[:kingdom], :phylum => params[:phylum], :t_class => params[:t_class], :t_order => params[:order], :family => params[:family])
        ...
        @taxonomy = "genus"
        @term = params[:genus]
        Species.where(:kingdom => params[:kingdom], :phylum => params[:phylum], :t_class => params[:t_class], :t_order => params[:order], :family => params[:family], :genus => params[:genus])
      end
