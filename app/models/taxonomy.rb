...
class Taxonomy < ActiveRecord::Base
  ...
  def self.kingdoms
    Species.select("distinct(kingdom)").map{ |x| x.kingdom}.delete_if{ |k| k.blank? || k == 'Unknown' }.map do |k|
      Taxonomy.find_by_name_and_hierarchy(k, 'kingdom')
    end
  end

  def self.phylums(kingdom)
    Species.select("distinct(phylum)").where(:kingdom => kingdom).map{|x| x.phylum}.delete_if{ |k| k.blank? || k == 'Unknown' }.map do |p|
      Taxonomy.find_by_name_and_hierarchy(p, 'phylum')
    end
  end

  def self.t_classes(phylum)
    Species.select("distinct(t_class)").where({:phylum => phylum}).map{|x| x.t_class}.delete_if{ |k| k.blank? || k == 'Unknown' }.map do |tc|
      Taxonomy.find_by_name_and_hierarchy(tc, 'class')
    end
  end

  def self.t_orders(t_class)
    Species.select("distinct(t_order)").where({:t_class => t_class}).map{|x| x.t_order}.delete_if{ |k| k.blank? || k == 'Unknown' }.map do |to|
      Taxonomy.find_by_name_and_hierarchy(to, 'order')
    end
  end

  def self.families(t_order)
    Species.select("distinct(family)").where({:t_order => t_order}).map{|x| x.family}.delete_if{ |k| k.blank? || k == 'Unknown' }.map do |f|
      Taxonomy.find_by_name_and_hierarchy(f, 'family')
    end
  end

  def self.genus(family)
    Species.select("distinct(genus)").where({:family => family}).map{|x| x.genus}.delete_if{ |k| k.blank? || k == 'Unknown' }.map do |g|
